...
fls <- list.files(path="./merged", pattern="*bam$",full.names=T,recursive = F)
fN=3
#
#
...
#dir.create("./merged/bedGraph")
#dir.create("./merged/RDS")
#dir.create("./merged/bedGraph/normalized")
#dir.create("./merged/bedGraph/enrichmentchipbwa")
#
...
#  saveRDS(normSampleCoverage,paste0("./merged/RDS/norm+8medsub_",fileName,".RDS"))
#
#  #save as a bedgraph file in the bedgraph directory
#  export(normSampleCoverage,paste0("./merged/bedGraph/normalized/norm+8medsub_",fileName,".bedGraph"))
#}
#
...
#  input<-readRDS(paste0("./merged/RDS/norm+8medsub_", fileName2, ".RDS") )
#  chip<-readRDS(paste0("./merged/RDS/norm+8medsub_", fileName1, ".RDS") )
#  #calculate enrichment
#  enrichment<- log2(chip/input)
#  saveRDS(enrichment,paste0("./merged/RDS/enrichmentsub", fileName1, ".RDS"))
#  export(enrichment,paste0("./merged/bedGraph/enrichmentchipbwa/enrichment+8medsub_", fileName1, ".bedGraph"))
#}
#
# get a list of all the files with the enrichment data
normCountsFiles<-list.files(path="./merged/bedGraph/enrichmentchipbwa/", "bedGraph")
...
normCounts<-import(paste0("./merged/bedGraph/enrichmentchipbwa/",normCountsFiles[f]))
...
pdf("./merged/merged2020110+8medsub.pdf",paper="a4r",width=11,height=8)
par(mfrow=c(2,2))
for (i in 1:length(tileList)) {
  tiles<-tileList[[i]]
...
pdf("./merged/pairwise_smScatterbwa+8medsub.pdf",paper="a4r",width=11,height=8)
par(mfrow=c(4,4))
for (i in 1:length(tileList)) {
  tiles<-tileList[[i]]
  samples<-names(mcols(tiles))
...
for (i in 1:length(winList)) {
  ...
  export(windows,paste0("./merged/bedGraph/smEnrich_",sampleName,"_",names(winList)[i],".bedGraph"))
}
}
